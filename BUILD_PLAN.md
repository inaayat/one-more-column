...
**Sources (normative):**
- [`ideation/CAPACITY_PLANNER_SPECIFICATION.md`](./ideation/CAPACITY_PLANNER_SPECIFICATION.md) — domain background + flexible platform vision (reference; not a delivery gate)
- [`ideation/HOSTED_APP_ARCHITECTURE.md`](./ideation/HOSTED_APP_ARCHITECTURE.md) — runtime / SoR / cutover (updated for Neon Auth + path proxy)
...
├── schemas/
│   ├── field_definitions.default.yaml ← first profile deliverable
...
1. Resolve open questions §10.1 and §10.4 (math + capacity model) with planning lead.  
2. Author `schemas/field_definitions.default.yaml` for the first planning profile.  
3. Extract pure `engines/*` from capacity math with golden fixtures.  
4. **H0 bootstrap:** scaffold `index.html` + `<base href>`, copy auth/lib patterns from `replacing-nerd-jobs`, add child `vercel.json`, open PR on main site for path + `omc-*` rewrites.  
5. Implement C1.1–C1.5 + **direct entry** on hosted stack once H1 is up.  
6. Ship **optional** `omc-import` (XLSX/CSV) + `omc-export` before any external integration work.  
7. **Do not** start Jira pull/publish or `omc-sync` until v1 success metrics are met.

---

*When implementation starts: ideation specs = domain reference; Hosted Arch = runtime bible; this Build Plan = sequencing & acceptance. Update all three when decisions land.*
