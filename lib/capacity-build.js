...
export async function buildCapacityForCycle({ cycleId, scenarioId, team, mode = 'due' }) {
  ...
  const assumptions = await db()`
    SELECT id, text, status, created_at FROM assumptions
    WHERE cycle_id = ${cycleId} AND status = 'active'
