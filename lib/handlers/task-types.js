...
async function loadTypesWithSteps(workspaceId) {
...
async function listTaskTypes(req, res) {
  ...
  const taskTypes = await loadTypesWithSteps(workspaceId);
...
async function createTaskType(req, res, auth) {
  ...
  `;
  sendJson(res, 201, { task_type: { ...rows[0], gate_templates: [] } });
}
...
async function patchTaskType(req, res, auth) {
  ...
  if (gateTemplates === undefined) {
    const steps = await db()`
      SELECT id, task_type_id, seq, label, duration_days, day_kind, dep_type, created_at, updated_at
      FROM gate_templates WHERE task_type_id = ${body.id} ORDER BY seq ASC
    `;
    gateTemplates = steps;
  }

  sendJson(res, 200, { task_type: { ...rows[0], gate_templates: gateTemplates } });
}
