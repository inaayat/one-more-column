...
/* Day-level tracking is deliberately absent: lib/handlers/capacity.js coerces
   anything that isn't `month` to `week`, so offering Day would promise columns
   the grid cannot draw. Tracked in FOLLOW_UPS.md. */
function granularityChoices(selected) {
  ...
  return `
    <div class="choice-row choice-row-2" role="radiogroup" aria-label="Track work by">
      ${options
        .map(
          ([value, name, desc]) => `
